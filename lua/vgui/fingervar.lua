...
local ConVar_MaxAngleX = CreateClientConVar( "finger_max_x", "100", false, false, "Maximum angle displacement on the finger's x axis" )
local ConVar_MaxAngleY = CreateClientConVar( "finger_max_y", "100", false, false, "Maximum angle displacement on the finger's y axis" )

local PANEL = {}

local MAX_ANGLE_X = ConVar_MaxAngleX:GetFloat()
local MAX_ANGLE_Y = ConVar_MaxAngleY:GetFloat()

cvars.AddChangeCallback("finger_max_x", function(cvar, old, new)
	local newAngle = tonumber(new)
	MAX_ANGLE_X = Either(newAngle ~= nil, newAngle, tonumber(old))
end)

cvars.AddChangeCallback("finger_max_y", function(cvar, old, new)
	local newAngle = tonumber(new)
	MAX_ANGLE_Y = Either(newAngle ~= nil, newAngle, tonumber(old))
end)
